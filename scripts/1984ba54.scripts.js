...
'use strict';
angular.module('fs').value('requestQueue', []).factory('requestExecutor', [
  'safeApply',
  'requestQueue',
  '$http',
  function (safeApply, requestQueue, $http) {
    var requestExecutor = {};
...
'use strict';
angular.module('fs').factory('host', [
  'config',
  function (config) {
    return config.get('host', '');
  }
]).value('method', 'GET').value('path', 'files').factory('requestBuilder', [
  'host',
  'method',
  'path',
  'requestExecutor',
  function (host, method, path, requestExecutor) {
...
]).constant('TOKEN_COOKIE_KEY', 'raml_token').value('shouldAutoRunInit', true).factory('requestTokenBuilder', [
  'config',
  '$cookies',
  'requestBuilder',
  'TOKEN_COOKIE_KEY',
  'shouldAutoRunInit',
  function (config, $cookies, requestBuilder, TOKEN_COOKIE_KEY, shouldAutoRunInit) {
    ...
    function init() {
      ...
      if (!token) {
        ...
        requestBuilder(shouldExecute).method('POST').path('token').success(function (data) {
          tokenProcessed = true;
...
angular.module('ramlEditorApp').constant('UPDATE_RESPONSIVENESS_INTERVAL', 800).service('ramlParserFileReader', [
  ...
  'ramlRepository',
  'safeApplyWrapper',
  function ($http, ramlParser, ramlRepository, safeApplyWrapper) {
